<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="奖学金" prop="jiangxuejin">
          <el-input v-model="ruleForm.jiangxuejin" 
              placeholder="奖学金" clearable  :readonly="ro.jiangxuejin"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="奖学金" prop="jiangxuejin">
              <el-input v-model="ruleForm.jiangxuejin" 
                placeholder="奖学金" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.tijiaowenjian" label="提交文件" prop="tijiaowenjian">
          <file-upload
          tip="点击上传提交文件"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.tijiaowenjian" label="提交文件" prop="tijiaowenjian">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.tijiaowenjian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingming = true;
          ...
          if(o=='jiangxuejin'){
            this.ruleForm.jiangxuejin = obj[o];
	    this.ro.jiangxuejin = true;
            continue;
          }
          if(o=='tijiaowenjian'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiangxuejintijiao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiangxuejintijiaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
